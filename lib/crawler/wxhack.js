...
module.exports = class {
    constructor(options) {
        ...
        this.phantom_polling_interval = options.phantom_polling_interval || 50;
    ...
    async init() {
        ...
        await Promise.all([
            ...
            this.page.on('onResourceError', error => {
                logger.verbose(`Request error: ${error.errorString} ${error.url}\t`);
            }),
            this.page.on('onResourceTimeout', data => {
                logger.verbose(`Request timeout: ${data.url}`);
            }),
            this.page.on('onConsoleMessage', msg => {
                logger.verbose(`Console: ${msg}`);
            }),
            this.page.on('onLoadStarted', status => {
                this.loading = true;
            }),
    ...
    async ensure_login() {
        ...
        if (!await this.page.evaluate(function () {
            ...
            while (true) {
                [verify_img, qr_code, error_tips] = await this.page.evaluate(function(username, password) {
                    var user = document.getElementById('account'),
                        pwd = document.getElementById('pwd'),
                        login = document.getElementById('loginBt'),
                        verify = document.getElementById('verifyImg'),
                    ...
                    if ((user && !user.value) || (pwd && !pwd.value)) {
                        user.value = username;
                        pwd.value = password;
                        login.click();
                    }
                ...
                if (qr_code)
                    ...
                    let verify_saved = path.join(this.media_dir, 'verify_img.png');
                ...
                await timeout(this.phantom_polling_interval);
            }
            let qrcode_saved = path.join(this.media_dir, 'qr_code.png');
    ...
    async reload() {
        ...
        await this.page.reload();
        await new Promise((resolve, reject) => this.client.once('loaded', resolve));
        logger.profile('reload()');
    }
